...
export default function ChildVaccination() {
  ...
  if (p.loading) {
    return <div className="max-w-4xl mx-auto px-6 py-20 flex items-center gap-2 text-slate-400"><Loader2 className="animate-spin" size={18}/> {t('Loading vaccination records…')}</div>
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-14">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-extrabold text-ink-900">{t('Child Vaccination')}</h1>
          <p className="mt-2 text-slate-500">{t('Vaccination history and immunization records for family dependents.')}</p>
        </div>
        <button onClick={() => setShowForm((v) => !v)} className="inline-flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white font-semibold px-5 py-3 rounded-full">
          <Plus size={18} /> {t('Add Dependent')}
        </button>
      </div>

      {showForm && (
        <form onSubmit={handleAddDependent} className="mt-8 border border-slate-100 rounded-2xl p-6 grid sm:grid-cols-3 gap-4">
          <input required placeholder={t('Name')} value={name} onChange={(e) => setName(e.target.value)} className="rounded-xl border border-slate-200 px-4 py-3" />
          <input type="number" placeholder={t('Age')} value={age} onChange={(e) => setAge(e.target.value)} className="rounded-xl border border-slate-200 px-4 py-3" />
          <input placeholder={t('Relationship (e.g. Son)')} value={relationship} onChange={(e) => setRelationship(e.target.value)} className="rounded-xl border border-slate-200 px-4 py-3" />
            ...
            return (
              <div key={dep.id}>
                {editingDepId === dep.id ? (
                  ...
                  >
                    <input autoFocus required placeholder={t('Name')} value={editDepForm.name} onChange={(e) => setEditDepForm({ ...editDepForm, name: e.target.value })} className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm" />
                    <input type="number" placeholder={t('Age')} value={editDepForm.age} onChange={(e) => setEditDepForm({ ...editDepForm, age: e.target.value })} className="w-24 rounded-lg border border-slate-200 px-3 py-1.5 text-sm" />
                    <input placeholder={t('Relationship')} value={editDepForm.relationship} onChange={(e) => setEditDepForm({ ...editDepForm, relationship: e.target.value })} className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm" />
                    <button type="submit" disabled={depRowSaving} className="p-1.5 text-emerald-600 hover:text-emerald-700" title={t('Save')}>
                      <Check size={16} />
                    </button>
                    <button type="button" onClick={() => setEditingDepId(null)} className="p-1.5 text-slate-400 hover:text-slate-600" title={t('Cancel')}>
                      <X size={16} />
                    </button>
                  </form>
                ) : (
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500">{dep.age} {t('Years')} · {dep.relationship}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => startEditDependent(dep)} className="p-1.5 text-slate-400 hover:text-brand-600" title={t('Edit')}>
                        <Pencil size={14} />
                      </button>
                      <button type="button" onClick={() => deleteDependent(dep)} className="p-1.5 text-slate-400 hover:text-red-600" title={t('Delete')}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
                ...
                <div className="mt-5 relative pl-8 border-l-2 border-brand-100 flex flex-col gap-5">
                  {vax.map((v) => (
                    <div key={v.id} className="relative">
                      ...
                      {editingVaxId === v.id ? (
                        ...
                        >
                          <input autoFocus required placeholder={t('Vaccine name')} value={editVaxForm.vaccine_name} onChange={(e) => setEditVaxForm({ ...editVaxForm, vaccine_name: e.target.value })} className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm" />
                          ...
                          <button type="submit" disabled={vaxRowSaving} className="p-1.5 text-emerald-600 hover:text-emerald-700" title={t('Save')}>
                            <Check size={16} />
                          </button>
                          <button type="button" onClick={() => setEditingVaxId(null)} className="p-1.5 text-slate-400 hover:text-slate-600" title={t('Cancel')}>
                            <X size={16} />
                          </button>
                        </form>
                      ) : (
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {v.status === 'completed' ? (
                            ...
                            <span className="inline-flex items-center gap-1 text-xs font-semibold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">
                              <Clock size={12} /> {t('Due Soon')}
                            </span>
                          )}
                          <button type="button" onClick={() => startEditVax(v)} className="p-1 text-slate-400 hover:text-brand-600" title={t('Edit')}>
                            <Pencil size={13} />
                          </button>
                          <button type="button" onClick={() => deleteVax(v)} className="p-1 text-slate-400 hover:text-red-600" title={t('Delete')}>
                            <Trash2 size={13} />
                          </button>
                        </div>
                      )}
                  ...
                  {vax.length === 0 && <p className="text-sm text-slate-400">{t('No vaccination records yet.')}</p>}
                </div>
                ...
                >
                  <Syringe size={14} /> {t('Add Vaccination')}
                </button>
                ...
                {openVaxFor === dep.id && (
                  <form onSubmit={(e) => handleAddVaccination(e, dep.id)} className="mt-3 border border-slate-100 rounded-2xl p-5 grid sm:grid-cols-3 gap-3">
                    <input required placeholder={t('Vaccine name')} value={vaxForm.vaccine_name} onChange={(e) => setVaxForm({ ...vaxForm, vaccine_name: e.target.value })} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm sm:col-span-2" />
                    ...
                    <div className="sm:col-span-3 flex gap-3">
                      <button type="submit" disabled={vaxSaving} className="bg-brand-600 hover:bg-brand-700 disabled:opacity-60 text-white text-sm font-semibold px-5 py-2 rounded-full">
                        {vaxSaving ? t('Saving…') : t('Save')}
                      </button>
                      <button type="button" onClick={() => setOpenVaxFor(null)} className="text-slate-500 text-sm font-medium px-5 py-2">{t('Cancel')}</button>
                    </div>
                  </form>
                )}
              </div>
