...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.session) {
        ...
        setNotice(t('Check your email to confirm your account, then sign in.'))
      }
    } catch (err) {
      setError(err.message || t('Could not create your account. Please try again.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-md mx-auto px-6 py-20">
      <h1 className="text-3xl font-extrabold text-ink-900">{t('Create your account')}</h1>
      <p className="mt-2 text-slate-500">{t('Own your records. See every connection in one place.')}</p>

      <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-4">
        <label className="flex flex-col gap-1.5">
          <span className="text-sm font-medium text-ink-900">{t('Full name')}</span>
          <input
            type="text"
            required
            value={fullName}
